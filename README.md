...
- Настраиваемое окно «тихих» часов для работы без уведомлений ночью.
...
QUIET_HOURS_START="00:00"  # Необязательно, обе переменные должны быть заданы
QUIET_HOURS_END="06:00"
TIMEZONE="Europe/Moscow"   # Необязательно
```

> Значения `NOTIFICATION_TIME`, `QUIET_HOURS_START/END` и `TIMEZONE` опциональны. По умолчанию уведомления отправляются в 09:00 UTC.
...
- Чтобы отключить уведомления ночью, задайте `QUIET_HOURS_START` и `QUIET_HOURS_END`. Если одна из переменных не указана, режим тишины не включается.
